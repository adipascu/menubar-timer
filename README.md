...
**Pebble.** The timer reaches a Pebble Time 2 through [TimerBar for Pebble](https://github.com/peblum/timerbar-for-pebble), a watchapp with an Android companion in between, since a Pebble pairs with a phone and not with a Mac. The app advertises itself on the local network with Bonjour and serves its state on port 47421 from `src/bridge.js`: the mode, the label, the seconds left, the menu's lengths and hints, the active category and the whole list with their shares, the goal standings, the focus totals for today, the week, the month and all time, and the battery. The Mac is the only place any of that is decided. Every snapshot carries the id of the running app and a revision that only goes up, so a companion can tell a stale message from a fresh one and can tell that the Mac restarted. The watch and the phone send requests and wait, they never change anything themselves, and the Mac rejects a length it does not offer, a category it does not have, and trims a label to the same forty characters its own dialog allows. The companion finds the Mac whenever both are on the same Wi-Fi, keeps an event stream open, and relays every change to the wrist, where the countdown runs on the watch's own clock and a long buzz marks Time's up. Picking a length on the watch starts the timer here, and a start with no label opens the same label prompt a menu click would, with the watch saying so until the label is typed on the phone or the Mac. The `Pebble:` line in the menu shows the six digit pairing code the companion has to be given once, and reads `Pebble: connected` while a phone is on the stream. When the Mac goes away the stream ends, and the phone and the watch stop the countdown and grey out rather than carry on with a state nothing is confirming.
...
116. /mr-polish and merge, make sure everything is merged in, do as many of these as needed.

    Make sure the companion android app, the watch and my mac are paired and completely in sync.

    Make sure to port over any new features from the mac app to the rest as needed.

    Make sure the phone app or watch can't make any updates by themsleves, all the info comes from the mac app and it's a single source of truth. If my mac is offline, disable the rest as well.

116 prompts. 2 multiple-choice answers. 0 lines of code written or edited by a human.
