...
const buildPrompt = ({ poolFile, feedbackFile, ideasFile, quizFile, library }) => {
  ...
  return [
    ...
    '',
    `2. Read ${feedbackFile}. It is a JSON object keyed by card title with the card's topic, how many times it has been shown, a status of "known" or "not-interested" where I marked one, an "interested" count where I asked for more like it, a "useful" count where a card taught me something, and "notes" where I typed something on the card, each with its text and the time I wrote it. Cards with a shown count are the ones I have actually seen. Leave not-interested cards out entirely, even when they also carry an interested count. Interested cards are the ones I asked for more of, so make sure they come up. Useful cards taught me something new, so check that it stuck. A note says what I wanted from a card, so honour it in the questions on that card and in the cards you write after.`,
    '',
    ...
    '',
    `7. When we stop, append this session to ${quizFile} in the shape above, creating the file if it is missing. Then update ${feedbackFile}: set "status": "known" on every card I clearly had, so the coach stops showing it, and leave the ones I missed alone so they keep coming round.`,
    '',
    cardsStep(
      8,
      cardsFile,
