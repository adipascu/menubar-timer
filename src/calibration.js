...
const buildPrompt = (tipsFile, feedbackFile, since) =>
  [
    ...
    '',
    `2. Read ${feedbackFile}. It is a JSON object keyed by tip title. Each entry carries the tip's topic, how many times it has been shown, and a status of "known" when I said I already knew it or "not-interested" when I said I did not want it. Known tips are learned, so build on them rather than repeating them. Not-interested tips tell you which angles to drop. Tips shown many times without a mark are the ones I keep sitting with, so those themes are still open.`,
    '',
...
export const createCalibration = (tipsFile, feedbackFile) => {
  ...
  const lastCalibratedAt = () => (existsSync(tipsFile) ? Math.max(seededAt, statSync(tipsFile).mtimeMs) : seededAt)
