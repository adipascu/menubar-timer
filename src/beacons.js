...
const PALETTES = {
  hackerspace: {
    ground: GROUND,
    edge: '#0f3a17',
    halo: 'rgba(57, 255, 20, 0.3)',
    bell: PHOSPHOR,
    bellHalo: 'rgba(57, 255, 20, 0.85)',
  },
  arcade: {
    ground: '#0b0b2a',
    edge: '#2a2a6a',
    halo: 'rgba(124, 245, 255, 0.35)',
    bell: '#ffe14a',
    bellHalo: 'rgba(255, 225, 74, 0.85)',
  },
  markets: {
    ground: '#0a0f1a',
    edge: '#1d2a44',
    halo: 'rgba(255, 176, 0, 0.3)',
    bell: '#ffb000',
    bellHalo: 'rgba(255, 176, 0, 0.85)',
  },
  cardio: {
    ground: '#140a0a',
    edge: '#4a1616',
    halo: 'rgba(255, 90, 60, 0.35)',
    bell: '#ff5a3c',
    bellHalo: 'rgba(255, 90, 60, 0.85)',
  },
  breathe: {
    ground: '#0e1a1c',
    edge: '#245a5e',
    halo: 'rgba(159, 232, 222, 0.3)',
    bell: '#9fe8de',
    bellHalo: 'rgba(159, 232, 222, 0.8)',
  },
  bloom: {
    ground: '#2a1a26',
    edge: '#6a3a5a',
    halo: 'rgba(255, 194, 214, 0.35)',
    bell: '#ffc2d6',
    bellHalo: 'rgba(255, 194, 214, 0.85)',
  },
  lofi: {
    ground: '#141824',
    edge: '#3a3f5a',
    halo: 'rgba(201, 184, 255, 0.3)',
    bell: '#c9b8ff',
    bellHalo: 'rgba(201, 184, 255, 0.8)',
  },
}
...
const tape = (canvas) => {
  ...
  const quoteText = (quote) =>
    `${quote.price.toFixed(2)} ${quote.change >= 0 ? '▲' : '▼'}${Math.abs(quote.change).toFixed(2)}%`
  let offset = width
  let lastTick = 0
  let hot = -1
  let hotUntil = 0
...
const candles = (canvas) => {
  ...
  let price = 100
  const bar = (open, close) => ({
    open,
    close,
    high: Math.max(open, close) + between(0, 1.5),
    low: Math.min(open, close) - between(0, 1.5),
  })
...
const splits = (canvas) => {
  ...
  return {
    frame(t, dt) {
      ...
      log.forEach((entry, i) =>
        label(ctx, entry, width - PAD - i * 74, 7, i === 0 && t < flashUntil ? '#ff5a3c' : '#a8756b', 'right'),
      )
...
const plain = (inner) => async (canvas) => ({ name: inner.name, ...(await inner(canvas)) })

const hacker = (inner) => async (canvas) => {
  const runner = await inner(canvas)
  return {
    ...runner,
    name: inner.name,
    frame(t, dt) {
...
const PRESETS = {
  hackerspace: {
    ...PALETTES.hackerspace,
    ambient: [matrixRain, decodeMarquee, scanLog].map(hacker),
    loud: hacker(spectrum),
  },
...
window.BEACONS = {
  ...
  async start(canvas, id, loudMusic) {
    ...
    let runner = null
    let note = null
    if (loudMusic && preset.loud) {
      try {
        runner = await preset.loud(canvas)
      } catch (error) {
        note = `the spectrum could not hear the system audio: ${error.name}: ${error.message}`
      }
    }
    runner ??= await pick(preset.ambient)(canvas)
