...
const feedbackStep = (number, feedbackFile) =>
  `${number}. Read ${feedbackFile}. It is a JSON object keyed by card title. Each entry carries the card's topic, how many times it has been shown, a status of "known" when I said I already knew it or "not-interested" when I said I did not want it, an "interested" count of how many times I asked for more like it, a "useful" count of how many times I said a card taught me something useful, and "notes", what I typed on the card when it came up, each with its text and the time I wrote it. Known cards are learned, so build on them rather than repeating them. Not-interested cards tell you which angles to drop, and that verdict wins over any interested count on the same card. Interested cards are the ones to go deeper on, the higher the count the more so: write what follows from them, the ideas next to them and the sources behind them. Useful cards are ones I learned from, which is not the same as wanting more of them: they show the depth, the sources and the kind of idea that actually lands for me, so write at that level, and treat what they taught as newly learned, building on it rather than repeating it. Every note is an instruction for this edition, whatever else the card is marked: answer it, explain what it asks about, or take the card where it points. Cards shown many times without a mark are the ones I keep sitting with, so those themes are still open.`
